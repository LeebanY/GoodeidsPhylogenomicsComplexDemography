#!/bin/bash
#SBATCH --job-name=getbams2    # Job name
#SBATCH --ntasks=16                    # Run on a single CPU
#SBATCH --mem=16gb                     # Job memory request
#SBATCH --partition=long
#SBATCH --output=indexbams2.log   # Standard output and error log
pwd; hostname; date
...
bwa mem $REF $loc/IF_1.fq.gz $loc/IF_2.fq.gz > IF.sam
...
samtools view -bS XC.sam > XC.bam
samtools view -bS XR.sam > XR.bam


samtools sort IF.bam -o IF.srt.bam
samtools sort XC.bam -o XC.srt.bam
samtools sort XR.bam -o XR.srt.bam


samtools rmdup -s IF.srt.bam IF.srt.rmdup.bam
samtools rmdup -s XC.srt.bam XC.srt.rmdup.bam
samtools rmdup -s XR.srt.bam XR.srt.rmdup.bam

samtools view -b -F 4 IF.srt.rmdup.bam > IF.mapped.bam
samtools view -b -F 4 XC.srt.rmdup.bam > XC.mapped.bam
samtools view -b -F 4 XR.srt.rmdup.bam > XR.mapped.bam
